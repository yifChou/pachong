    ...
    def singer2db(self, id, sleep=1):
        ...
        if singers is not None:
            for singer in singers:
                print(singer)
                session.add(moduelsinger.Singer(singer=singer[1], singerid=singer[0], type=self.mukuai,
                                                creattime=mytime.get_now()))
                session.commit()
        time.sleep(sleep)
    ...
    def music2db(self, id, t=5):
        ...
        if comments != 0 or comments is not None:
            ...
            if mus is not None:
                ...
                # print(singer,album)
                session.add(
                    moduelsinger.Music(id=uuid.uuid3(uuid.uuid4(), "1"), musicid=mus["@id"], singerid=self.mukuai,
                                       title=mus["title"], image=mus["images"], singer=singer,
                                       album=album,
    ...
    def get_music(start, end):
        ...
        for sid in range(start, end):
            ...
            if musics is not None:
                ...
                for music in musics:
                    try:
                        comments = pc.get_comment(music[0])
                        if comments != 0 or comments is not None:
                            mus = pc.get_name(music[0])
                            print(mus, comments)
                            if mus is not None:
                                desc = mus["description"]
                                singer = desc[desc.find("歌手：") + 3:desc.find("所属专辑")]
                                ...
                                # print(singer,album)
                                session.add(
                                    moduelsinger.Music(id=str(uuid.uuid3(uuid.uuid4(), "1")), musicid=mus["@id"],
                                                       singerid=pc.mukuai,
                                                       title=mus["title"], image=mus["images"], singer=singer,
                                                       album=album,
                                                       pubdate=mus["pubDate"], comments=comments,
                                                       creattime=mytime.get_now(),
                                                       updatetime=mytime.get_now()))
                                lock.acquire()
                                session.commit()
                                lock.release()
                                print(mus["title"] + "入库成功:id=", music[0])
                                time.sleep(t)
